...
public class BookPriceDAO {
    ...
    public List<String> getAllBookPrices() throws SQLException {
        ...
        String query = "SELECT b.id, b.title, bp.price FROM books b JOIN book_prices bp ON b.id = bp.book_id ORDER BY bp.price DESC";
        
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery(query)) {
            while (rs.next()) {
                String price = String.format("Sách: %s | Giá: %.0f",
                    rs.getString("title"),
                    rs.getDouble("price")
                );
                prices.add(price);
    ...
    public List<String> getBooksByPriceRange(double minPrice, double maxPrice) throws SQLException {
        ...
        String query = "SELECT b.id, b.title, bp.price FROM books b JOIN book_prices bp ON b.id = bp.book_id WHERE bp.price BETWEEN ? AND ? ORDER BY bp.price";
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    String book = String.format("Sách: %s | Giá: %.0f",
                        rs.getString("title"),
                        rs.getDouble("price")
                    );
                    books.add(book);
    ...
    public String getCheapestBook() throws SQLException {
        String query = "SELECT b.id, b.title, bp.price FROM books b JOIN book_prices bp ON b.id = bp.book_id ORDER BY bp.price ASC LIMIT 1";
        
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery(query)) {
            if (rs.next()) {
                return String.format("Sách rẻ nhất: %s | Giá: %.0f",
                    rs.getString("title"),
                    rs.getDouble("price")
                );
            }
    ...
    public String getMostExpensiveBook() throws SQLException {
        String query = "SELECT b.id, b.title, bp.price FROM books b JOIN book_prices bp ON b.id = bp.book_id ORDER BY bp.price DESC LIMIT 1";
        
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery(query)) {
            if (rs.next()) {
                return String.format("Sách đắt nhất: %s | Giá: %.0f",
                    rs.getString("title"),
                    rs.getDouble("price")
                );
            }
