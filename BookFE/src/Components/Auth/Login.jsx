...
export default function Login({
  ...
  const [form, setForm] = useState({ email: "", password: "" }); // ← đổi username → email
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await login(form.email, form.password); // ← đổi
      localStorage.setItem("token", res.data.token);
      localStorage.setItem("user", JSON.stringify(res.data.user));
      const user = res.data.user;
      onSuccess(user); // App.jsx tự navigate sau khi onSuccess
    } catch (err) {
  ...
  const styles = {
    ...
    links: { display: "flex", gap: 12, alignItems: "center", marginTop: 12 },
    linkBtn: {
      background: "none",
      border: "none",
      color: "#2563eb",
